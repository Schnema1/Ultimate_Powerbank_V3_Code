...
#include <Mux.h>                // ad-mux-lib-master
#include <FastLED.h>            // LED RING libraries
#include <X9C.h>                // digital pot, for changing charging current
#include <Adafruit_GFX.h>       // display graphics
#include <TM1640.h>             // LED matrix chip library
...
// DEVICE NAMES AND CORRESPONDING PINS
...
// 100K NTC THERMISTER VALUES
float R1 = 10000;
float logR2, R2, T;
float c1 = 1.009249522e-03, c2 = 2.378405444e-04, c3 = 2.019202697e-07;

// VOLTAGE MEASURING VALUES
...
void loop()
{
  ...
  if (matrixShortON == true && currentMillis - matrixOnTIME > matrixOnDELAY)
  { // in matrix activated for a short time, turn it off
    displayOff ();
    matrixShortON = false;
  ...
  if (idleTime >= idlePowerOFF)
  { // if powerbank is idle for longer than idle power off seconds, SHUT DOWN
  ...
  { // begin power button pressed actions
  ...
  if (powerButton.isReleased())
  {
    ...
    {
      if (flashlightMode == false && matrixShortON == false)
      { // if flashlight mode is false, display voltages on matrix
        mapVoltages ();
        matrixDisplayPRECENT ();    // display battery precentage on led matrix
      ...
      { // if flashlightmode is true, switch mode
        ledMode++;
  ...
  if (powerButtonSTATE == 0 && powerHoldTIME >= powerOffDURATION)
  { // if power button is held for longer than x mili sec, POWER OFF
    Serial.print("POWERING OFFFFFFFFFFFFFFFFFFF");
    powerOff ();
  } // end all power button actions


  if (usbButton.isPressed())
  { // begin usb button pressed actions
  ...
  if (wirelessButton.isPressed())
  { // begin wireless button actions
  ...
  if (wirelessOn == true && inWirelessMODE == false)
  { // Wireless on actions (once)
    lineOneTEXT = ("ON");
    lineTwoTEXT = ("WIR");
  ...
  if (wirelessOn == false && inWirelessMODE == true)
  { // Wireless off actions (once)
    lineOneTEXT = ("OFF");
    lineTwoTEXT = ("WIR");
    lastDisplayTEXT = millis ();
  ...
  if (inWirelessMODE == true)
  { // breath blue led while charging
  ...
  if (usbQcON == true && inQcMODE == false)
  { // Qc on actions (once)
    lineOneTEXT = ("ON");
    lineTwoTEXT = ("USB");
    lastDisplayTEXT = millis ();
  ...
  if (usbQcON == false && inQcMODE == true)
  { // Qc off actions (once)
    lineOneTEXT = ("OFF");
    lineTwoTEXT = ("USB");
    lastDisplayTEXT = millis ();
  ...
  if (moduleOn == true && inModuleMODE == false)
  { // module on actions (once)
    lineOneTEXT = ("ON");
    lineTwoTEXT = ("EXTR");
    lastDisplayTEXT = millis ();
  ...
  if (moduleOn == false && inModuleMODE == true)
  { // module off actions (once)
    lineOneTEXT = ("OFF");
    lineTwoTEXT = ("EXTR");
    lastDisplayTEXT = millis ();
  ...
  if (moduleOn == true || usbQcON == true || wirelessOn == true)
  { // display battery percentage periodocally
  ...
  {
    if (currentMillis - lastDisplayTEXT <= 2000)
    { // display texts for two seconds
      displayTexts();
    } // end display texts
  ...
  if (ringVoltageON == true && currentMillis - firstOnTIME >= 3000)
  { // after interval, turn off all LEDs and display
    LEDFadeBLACK ();
    displayOff ();
    turnOnFINISHED = true;
    ringVoltageON = false;
  ...
  { // action to do when input voltage is in range
    ...
    if (batteryTemp <= 40 && chargingTemp <= 55)
    { // if temps are low, use full charging speed
      if (inputVoltage < 6)
      { // if voltage is less than 6V use charge using 10W
        pot.setPot(10);
      } // end first 10W charging mode

      if (inputVoltage >= 6 && inputVoltage < 14)
      { // 15W charging mode
        pot.setPot(20);
      } // end 15W charging mode

      if (inputVoltage >= 14 && inputVoltage < 17)
      { // 30W charging mode
        pot.setPot(25);
      } // end 30W charging mode

      if (inputVoltage >= 17 && inputVoltage < 21)
      { // 40W charging mode
        pot.setPot(35);
      } // end 40W charging mode

      if (inputVoltage >= 21 && inputVoltage < 28)
      { // 60W charging mode
        pot.setPot(54);
  ...
  if (overCurrentPROTEC ==  true || overTempPROTEC == true || overVoltPROTEC == true || batteryFaultPROTEC == true)
  { // activate warninng LED if system exceedes nominal value
    displayTexts ();
...
void readSensors()
{
  ...
  if ( moduleConnectRAW > 700 )
  { // compute module connected or not
    moduleConnect = 1;
  }
  else {
    moduleConnect = 0;
...
{
  if (powerbankAmp > currentLimit)
  { // if powerbank exceeds current limit, trigger overcurrent protection
  ...
  if (batteryTemp > 52 || caseTemp > 65 || chargingTemp > 66 || moduleTemp > 65)
  { // if these temperatures (in degrees C) are exceded, trigger over temp protection
    outputOff ();
    lineOneTEXT = ("OVER");
    lineTwoTEXT = ("TEMP");
    lastDisplayTEXT = millis ();
  ...
  if (batteryVoltage < minBatteryVOLTAGE || batteryVoltage > (maxBatteryVOLTAGE + 1))
  { // if battery is under min voltage, trigger under voltage protection.
  ...
  if (batteryVoltage < emptyBatteryVOLTAGE && batteryVoltage > minBatteryVOLTAGE)
  { // low power shut off
    outputOff ();
    lineOneTEXT = ("LOW");
    lineTwoTEXT = ("BATT");
  ...
  if (inputVoltageLOW == false && inputVoltage < 28 && batteryFaultPROTEC == false && overTempPROTEC == false && chargeCurrentLOW == false )
  { // if input voltage is over 4.5V, and if over 28V, dont charge to protect electronics
    chargingMode = true;
  }
  else {
    chargingMode = false;
  ...
  if (moduleTemp > 60)
  { // if module temp is grater than x, turn it off
    moduleOn = false;
    digitalWrite(modulePin, LOW);
  ...
  if (fanOn == false && higherTemp > 42)
  { // if temperature is lower than fanOnTEMP, turn off fan
    digitalWrite (fanPin, HIGH);
    fanOn = true;
  }

  if (fanOn == true && higherTemp < 37)
  { // if temperature is lower than fanOnTEMP, turn off fan
    digitalWrite (fanPin, LOW);
    fanOn = false;
  } // end fan on or off control

} // end interpret values

/*
   DISPLAY TEXT ----------------------------------------------------------------
*/
void displayTexts ()
{ // display two line texts for warning, status and power off/on
...
*/
void mapVoltages ()
{ // map voltages onto display value
...
/*
   LED RING DISPLAY VOLTAGE ----------------------------------------------------
*/
void LEDRingDisplayVOLTAGE ()
{ // LED ring display voltage
...
/*
   MATRIX DISPLAY PERCENT ------------------------------------------------------
*/
void matrixDisplayPRECENT ()
{ // displays battery precentage onto matrix
  ...
  { // center screen if less than 10%
    matrix.setCursor(5, 5);
  } // end if
  if (batteryPercentage >= 10 && batteryPercentage < 100)
  { // center screen if between 10 and 100 percent
    matrix.setCursor(3, 5);
  } // end if
  if (batteryPercentage == 100)
  { // center screen if 100 percent
